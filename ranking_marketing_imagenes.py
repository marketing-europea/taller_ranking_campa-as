...
DEFAULT_EXCLUDED_ASESOR_CODES = "28005, 34400, 29403, 105-0000, 48400, 08417-001, 11013, 436, 110-G010, Sin codigo, Sin codigo red"
...
def parse_date_text(text: str, label: str) -> date:
    ...
    try:
        ...
        raise ValueError(f"{label} debe tener formato DD/MM/AAAA. Ejemplo: 31/05/2026") from exc
...
def filter_base(
    ...
    if fecha_efecto_maxima is not None:
        mask = mask & df["FECHA_EFECTO"].notna() & df["FECHA_EFECTO"].dt.date.le(fecha_efecto_maxima)
...
def format_for_display(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in out.columns:
        if col in MONEY_COLUMNS:
            out[col] = out[col].apply(lambda x: f"{float(x):,.2f} €".replace(",", "X").replace(".", ",").replace("X", "."))
        elif col in PERCENT_COLUMNS:
            out[col] = out[col].apply(lambda x: f"{float(x):.2%}".replace(".", ","))
    return out
...
# ============================================================
# Imagen campaña Fórmula 1 - formato horizontal para email
# ============================================================
...
def text_width(draw: ImageDraw.ImageDraw, text: object, font: ImageFont.ImageFont) -> int:
    box = draw.textbbox((0, 0), str(text), font=font)
    return box[2] - box[0]


def draw_text_fit(draw: ImageDraw.ImageDraw, xy: tuple[int, int], text: object, max_width: int, font_size: int, fill, bold: bool = False) -> None:
    text = str(text)
    size = font_size
    font = get_font(size, bold=bold)
    while size > 9 and text_width(draw, text, font) > max_width:
        size -= 1
        font = get_font(size, bold=bold)
...
def make_checkered_background(width: int, height: int) -> Image.Image:
    square = 95
    ...
    for y in range(-square, height + square, square):
        for x in range(-square, width + square, square):
            is_black = ((x // square) + (y // square)) % 2 == 0
            color = (18, 18, 20) if is_black else (238, 238, 238)
            poly = [(x + 18, y), (x + square + 18, y), (x + square - 12, y + square), (x - 12, y + square)]
            draw.polygon(poly, fill=color)
    bg = bg.filter(ImageFilter.GaussianBlur(1.3))
    overlay = Image.new("RGBA", (width, height), (4, 6, 10, 112))
    return Image.alpha_composite(bg.convert("RGBA"), overlay)


def generate_formula1_email_image(ranking: pd.DataFrame, fecha_desde: date, fecha_hasta: date, ranking_por: str, metric_label: str, max_rows: int = 6) -> bytes:
    if ranking.empty:
        raise ValueError("No hay datos para generar la imagen de campaña.")

    width, height = 1200, 520
    img = make_checkered_background(width, height)
    draw = ImageDraw.Draw(img)

    red = (242, 30, 45, 255)
    black = (20, 22, 26, 255)
    white = (255, 255, 255, 255)
    soft_white = (235, 235, 235, 255)
    grey = (195, 198, 204, 255)
    gold = (255, 203, 92, 255)
    silver = (210, 214, 220, 255)
    bronze = (206, 140, 86, 255)

    draw.rounded_rectangle((28, 24, width - 28, height - 24), radius=34, fill=(10, 12, 16, 225), outline=(255, 255, 255, 60), width=2)

    draw.rounded_rectangle((52, 50, 390, 470), radius=28, fill=(0, 0, 0, 188))
    draw_text_fit(draw, (82, 78), "RANKING", 280, 48, white, bold=True)
    draw_text_fit(draw, (82, 130), "DECESOS", 280, 50, red, bold=True)
    draw.rectangle((82, 193, 315, 203), fill=red)
    draw_text_fit(draw, (82, 225), "CAMPAÑA", 260, 25, grey, bold=True)
    draw_text_fit(draw, (82, 258), "FÓRMULA 1", 260, 39, white, bold=True)
    periodo = f"{fecha_desde.strftime('%d/%m/%Y')} - {fecha_hasta.strftime('%d/%m/%Y')}"
    draw_text_fit(draw, (82, 321), periodo, 260, 23, soft_white, bold=True)
    draw_text_fit(draw, (82, 357), metric_label.upper(), 260, 21, grey, bold=True)
    draw.rounded_rectangle((82, 405, 325, 445), radius=16, fill=red)
    draw_text_fit(draw, (111, 414), "EUROPEA SEGUROS", 190, 20, white, bold=True)

    table_x, table_y = 420, 58
    table_w, table_h = 500, 405
    draw.rounded_rectangle((table_x, table_y, table_x + table_w, table_y + table_h), radius=22, fill=(255, 255, 255, 238))
    draw.rounded_rectangle((table_x, table_y, table_x + table_w, table_y + 52), radius=22, fill=red)
    draw.rectangle((table_x, table_y + 25, table_x + table_w, table_y + 52), fill=red)

    headers = [("POS", 18, 62), ("AGENCIA", 88, 145), ("PROV.", 248, 94), ("DECESOS", 358, 120)]
    for label, offset, maxw in headers:
        draw_text_fit(draw, (table_x + offset, table_y + 15), label, maxw, 20, white, bold=True)

    top = ranking.head(max_rows).copy()
    row_h = 56
    for idx, (_, row) in enumerate(top.iterrows(), start=1):
        y = table_y + 52 + (idx - 1) * row_h
        fill = (248, 249, 252, 255) if idx % 2 else (232, 235, 240, 255)
        ...
        if medal:
            draw.ellipse((table_x + 20, y + 12, table_x + 54, y + 46), fill=medal)
            draw_text_fit(draw, (table_x + 31, y + 17), idx, 20, 18, black, bold=True)
        else:
            draw_text_fit(draw, (table_x + 31, y + 15), idx, 35, 22, black, bold=True)
        ...
        agencia_txt = str(nombre).strip() or str(codigo)
        draw_text_fit(draw, (table_x + 88, y + 14), agencia_txt, 150, 22, black, bold=True)
        draw_text_fit(draw, (table_x + 248, y + 14), provincia, 95, 21, black, bold=False)
        draw_text_fit(draw, (table_x + 358, y + 14), format_int_es(importe), 120, 23, black, bold=True)

    top1 = ranking.iloc[0]
    pole_x, pole_y = 948, 58
    draw.rounded_rectangle((pole_x, pole_y, 1148, 292), radius=26, fill=(8, 9, 12, 240), outline=(255, 255, 255, 70), width=2)
    draw.rounded_rectangle((pole_x + 18, pole_y + 18, pole_x + 182, pole_y + 58), radius=14, fill=red)
    draw_text_fit(draw, (pole_x + 37, pole_y + 27), "POLE POSITION", 130, 17, white, bold=True)
    pole_name = str(top1.get("NOMBRE", top1.get("CODIGO", "")))
    pole_code = str(top1.get("CODIGO", ""))
    pole_amount = format_int_es(top1.get("FACTURACION_NETA", 0))
    draw_text_fit(draw, (pole_x + 24, pole_y + 83), pole_code, 150, 46, white, bold=True)
    draw_text_fit(draw, (pole_x + 24, pole_y + 136), pole_name, 152, 22, soft_white, bold=True)
    draw_text_fit(draw, (pole_x + 24, pole_y + 183), f"{pole_amount} €", 155, 33, gold, bold=True)

    total_fact = ranking["FACTURACION_NETA"].sum() if "FACTURACION_NETA" in ranking.columns else 0
    total_pol = ranking["POLIZAS_GRABADAS"].sum() if "POLIZAS_GRABADAS" in ranking.columns else 0
    draw.rounded_rectangle((948, 315, 1148, 462), radius=24, fill=(255, 255, 255, 235))
    draw_text_fit(draw, (972, 337), "RESUMEN", 150, 22, black, bold=True)
    draw.rectangle((972, 366, 1128, 371), fill=red)
    draw_text_fit(draw, (972, 388), "Fact. neta", 88, 18, black, bold=True)
    draw_text_fit(draw, (1060, 388), format_int_es(total_fact), 70, 18, red, bold=True)
    draw_text_fit(draw, (972, 420), "Pólizas", 88, 18, black, bold=True)
    draw_text_fit(draw, (1060, 420), int(total_pol), 70, 18, red, bold=True)
...
st.caption("Constructor flexible para generar rankings por agente o asesor usando facturación y anulaciones de Decesos.")
...
with st.sidebar:
    ...
    facturacion_asesor_file = st.file_uploader("FACTURACION_DECESOS_ASESOR (solo si ranking por asesor)", type=["xls", "xlsx", "xlsm"])
...
try:
    ...
    st.error("Falta una librería para leer Excel antiguo .xls. Instala dependencias con: pip install -r requirements.txt")
    st.exception(error)
    st.stop()
...
for preferred in ["POLIALTA", "FECHA GRABACION", "FECHA_GRABACION"]:
...
with st.form("ranking_form"):
    ...
    with c1:
        date_column = st.selectbox("¿Qué fecha quieres usar para el rango?", date_cols, index=default_date_index)
    ...
    with c2:
        ...
        usar_fecha_efecto_maxima = st.checkbox("Fijar fecha de efecto máxima")
    with c3:
        fecha_efecto_maxima_text = st.text_input(
            ...
            help="Formato DD/MM/AAAA. Ejemplo: 31/05/2026 o 01/07/2026.",
        )
    ...
    with c5:
        ...
        generar_imagen_formula1 = st.checkbox(
            "Generar imagen campaña Fórmula 1 para email",
            value=True,
        )
    with c6:
        asesor_codes_text = st.text_area("Códigos excluidos en ranking asesor", value=DEFAULT_EXCLUDED_ASESOR_CODES, height=80)
        asesor_prefixes_text = st.text_input("Prefijos excluidos en ranking asesor", value=DEFAULT_EXCLUDED_ASESOR_PREFIXES)
...
if submitted:
    try:
        ...
        k1.metric("Facturación neta", f"{ranking['FACTURACION_NETA'].sum():,.2f} €".replace(",", "X").replace(".", ",").replace("X", ".") if not ranking.empty else "0,00 €")
        k2.metric("Pólizas grabadas", int(ranking["POLIZAS_GRABADAS"].sum()) if not ranking.empty else 0)
        k3.metric("Pólizas anuladas", int(ranking["POLIZAS_ANULADAS"].sum()) if not ranking.empty else 0)
        churn_total = ranking["POLIZAS_ANULADAS"].sum() / ranking["POLIZAS_GRABADAS"].sum() if not ranking.empty and ranking["POLIZAS_GRABADAS"].sum() else 0
        k4.metric("Churn pólizas", f"{churn_total:.2%}".replace(".", ","))
        ...
        parametros = {
            "ranking_por": ranking_por,
            "fecha_columna": date_column,
            "fecha_desde": fecha_desde.strftime("%d/%m/%Y"),
            "fecha_hasta": fecha_hasta.strftime("%d/%m/%Y"),
            "fecha_efecto_maxima": fecha_efecto_maxima_valor.strftime("%d/%m/%Y") if fecha_efecto_maxima_valor else "No aplicada",
        ...
        if generar_imagen_formula1:
            st.subheader("Imagen campaña Fórmula 1 para email")
            imagen_formula1 = generate_formula1_email_image(
                ranking=ranking,
                fecha_desde=fecha_desde,
                fecha_hasta=fecha_hasta,
                ranking_por=ranking_por,
            ...
            st.image(
                imagen_formula1,
                caption="Banner horizontal listo para insertar en correo",
                use_container_width=True,
            )
            st.download_button(
                "Descargar imagen Fórmula 1 para email",
                data=imagen_formula1,
                file_name="ranking_formula1_email.png",
                mime="image/png",
            )
